$(function () {
    ...
    window.doNotification = function (type, nid, title, message, timeout, icon, actions, hints, onAction, onClose) {
        ...
        var a = $('<div id="' + nID + '" class="alert ' + type + '">' + '<img class="notification_icon" id="notification_icon' + nID + '"></img>' + '<span class="title">' + title + "</span>" + '<span class="message">' + message + "</span>" + '<div class="dismiss">&#215;</div>' + "</div>");
        ...
        $(".notifications").prepend(a);
        if (icon) {
            var encoding = icon[0], img_data = icon[3];
            if (encoding == "png") {
