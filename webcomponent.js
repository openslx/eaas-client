...
class EaasClientElement extends HTMLElement {
    ...
    async pressKey(
        ...
        { altKey, ctrlKey, metaKey, timeout = 100 } = {}
    ) {
    ...
    async play() {
        ...
        const containerId =
            this.getAttribute("container-id")?.match(/\/?([^/]+)$/)[1];
        ...
        if (enableInternet) {
            ...
            if (!internetDate) {
                ...
                networkBuilder.getNetworkConfig().archived_internet_date =
                    internetDate;
                await networkBuilder.enableDhcpService(
                    networkBuilder.getNetworkConfig()
                );
            }
        ...
        if (networkId) {
            ...
            const { componentId } =
                client.network.networkConfig.components.find(
        ...
        if (this.session == null) this.session = client.getActiveSession();
